        ...
        <div class="search2">
          <div class="searchBox1">
            <div class="searchInput">
              <input ref="input1" @keyup.13="searcher()" v-model="searchContent" type="text" placeholder="在此输入您搜索的内容" onfocus="this.placeholder=''" onblur="this.placeholder='在此输入您搜索的内容'">
            </div>
      ...
      <div class="SearchContent">
        <div class="SearchHead">
          <div class="SearchContent1 row">
            ...
            <ul>
              <li :class="{'asc':isAsc1,'desc':isDesc1}" @click="publishingTime();Searcher1();inf()">
                出版时间
                <span>
              ...
              <li :class="{'asc':isAsc3,'desc':isDesc3}" @click="relevant();Searcher1();inf()">
                相关度
                <span>
            ...
            <div class="SearchHeadThree col-md-3 col-sm-2">
              <ul>
                ...
                <!-- <li><img src="../../public/img/right.png" alt="导出" title="导出"></li> -->
        ...
        <div class="SearchBody1 row">
          ...
          element-loading-background="f5f5f5">
            <div class="matching" v-if="a1!=''">
              <p class="title3" @click="toJournal(journal1.nlmId,journal1.course)"><span v-html="journal1.fullName"></span><span style="color:blue"> ( </span><span v-html="journal1.subName"></span><span style="color:blue"> )</span></p>
                <div class="Periodical1"> 
                  <div class="Periodical1Left">
                    <div>
                      <p><span v-if="journal1.course!=null">{{journal1.course}}</span><span v-else>N/A</span><span v-show="journal1.area!='未录'">{{journal1.area}}</span><span v-if="journal1.subCourse!=null">{{journal1.subCourse}}</span><span v-else>N/A</span></p>
                      <p><span @click="officialUrl(journal1.officialUrl)" v-if="journal1.officialUrl!=null">杂志入口</span><span v-else>N/A</span><span  @click="nlmId1(journal1.nlmId);searcher()">最新文章</span></p>
                    </div>
                    <div class="issn">
                      <div>
                        <p>ISSN</p>
                        <p v-if="journal1.issn!=null">{{journal1.issn}}</p>
                        <p v-else>N/A</p>
                      </div>
                      <div>
                        <p>发布国家</p>
                        <p v-if="journal1.countryZh!=null">{{journal1.countryZh}}</p>
                        <p v-else>N/A</p>
                      </div>
                      <div>
                        <p>投稿难度</p>
                        <p v-if="journal1.enterPresent!=null">{{journal1.enterPresent}}</p>
                        <p v-else>N/A</p>
                      </div>
                      <div>
                        <p>投稿周期</p>
                        <p v-if="journal1.acceptTime!=null">{{journal1.acceptTime}}</p>
                        <p v-else>N/A</p>
                      </div>
                      <div>
                        <p>发布频次</p>
                        <p v-if="journal1.publishingCycle!=null">{{journal1.publishingCycle}}</p>
                        <p v-else>N/A</p>
                      </div>
                    </div>
                  </div>
                  <div class="Periodical1Right">
                    <div id="Period1" :style="{width: '270px', height: '180px'}"></div>
                  </div>
                </div>
            </div>
            <div class="hengxian1" v-if="a1!=''">
              <span>相似文章</span>
              <div></div>
            </div>
            <ul v-if="a1!=''">
...
export default {
  metaInfo() {
    return {title: this.search + '-相似文章'} // set a title
  },
  data() {
    return {
      ...
      sort:'',shoucang1:require('../../public/img/shoucang.png'),shoucang2:'收藏',shoucang10:require('../../public/img/shoucangactive.png'),phone:"",shoucang11:"取消收藏",pmidList:''
    }
  },
  components:{
    Header,Footer
  ...
  computed: {
    ...
    data2: {
      get: function(){
        return this.journal1; // 在这里把临时对象的值通过计算属性赋值给页面中用到的对象
      }
    },
    data5:{
      get:function(){
  ...
  methods:{
    ...
    //杂志影响因子图
    Period1(){
      let a=this.journal1.ifs;
      let d=[]
      for(let b in a){
        d.push(a[b])
      }
      var echarts1 =[];echarts1= document.getElementById("Period1");
      // 绘制图表
      if(echarts1){
    ...
    influencefactorDown(){
      ...
      this.$router.push({
        path:'/Resemble',
        query:{
          chk2:this.chk2,chk3:this.chk3,chk5:this.chk5,sort:this.sort,page:this.currentPage,search:this.search,Id:this.nlmId
        }
      })
    },
    yearDown(){
    ...
    Searcher1(){
      ...
      this.axios.post(`/pubmed/moreLikeThisPubmedArticleJournalAll/`, qs.stringify({
        ...
        if((result.data.res.journalList.journalTermsAggs).length==0){
          ...
          this.a1=this.searchPubmedArticle.res.journalList.journalTermsAggs[0].journalTopHits.searchData;
          this.journal1=this.a1[0].journal
          this.result=this.searchPubmedArticle.res.journalList.journalTermsAggs[0].count;
          this.timer=this.searchPubmedArticle.res.journalList.searchTime;
        }
        if(!this.result){
          this.el_show=false;
          this.result=0
...
<style scoped>
  ...
  .SearchBody>p>span:nth-child(1)>span:nth-child(2){
    margin-left: 15px;
  }
